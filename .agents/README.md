...
| `bun run jira:sync-issues` | Pull Jira Epics/Stories into `.context/PBI/` markdown files (content sync, not metadata). Subcommands: `get <KEY> --include-comments` (single issue — ALL custom fields + comments), `jql "<query>"` (batch), `status`, `pull` (with `--epic <KEY>`, `--story <KEY>`, `--include-comments`, `--dry-run`, `--json`). `get` / `jql` are the canonical detailed-content read path (replacing `acli view`, which returns null for `customfield_*`). |
