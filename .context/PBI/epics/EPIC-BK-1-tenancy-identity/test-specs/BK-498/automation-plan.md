...
| Risk | Mitigation |
|---|---|
| ~~TC7's non-member-actor binding semantics unconfirmed for the Modules-family case~~ **RESOLVED 2026-08-23** | The bound-mint approach (`workspace_id: WORKSPACE_NOT_MEMBER_ID`) 403s at MINT time (`assertTokenIssuanceAuthorized` requires membership for ANY `workspace_id`), so it was structurally unreachable. Fix: mint UNBOUND (`workspace_id` omitted — no membership check applies) via a brand-new real identity (`STAGING_NON_MEMBER_EMAIL` = `bk569-nonmember@ambuusteln.resend.app`, provisioned via signup+resend-cli OTP+confirm, never invited to any workspace) signing in through `auth.authenticateSuccessfully(config.testNonMember)`. The `/modules` route's own membership gate is RLS-on-INSERT, not token-binding — live-verified this reproduces `403 { code: 'forbidden', details.reason: 'not_a_member' }` against `BK264_DEFECT_TRIAGE_PROJECT_ID`. See `atc/BK-569-reject-module-creation-non-member.md`. |
