...
## Refined Acceptance Criteria — BK-205 (Shift-Left DRAFT, updated 2026-07-24 — 7 of 9 pending items closed by inference from already-made decisions)
...
Feature: TMS-Milestone — Create a milestone with a target date
  ...
  # STILL OPEN — no existing decision answers this; genuinely needs a PO/Dev number or explicit "no cap"
  @boundary @low @needs-po-confirmation
  Scenario: Should accept a target date far in the future with no stated upper bound
    ...
    Then the milestone is created and listed, unless PO confirms an upper bound should exist
  ...
  # STILL OPEN — kept open deliberately even though Backend's index technically implies "distinct":
  # whether two near-identical names should be ALLOWED as a product decision is a separate question from what the index does.
  @positive @medium @needs-po-confirmation
