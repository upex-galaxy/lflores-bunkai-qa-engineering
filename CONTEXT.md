...
```
.context/
├── PRD/                       → Product Requirements (generated by /project-discovery Phase 2)
├── SRS/                       → Software Requirements (generated by /project-discovery Phase 2)
├── PBI/                       → Per-module + per-ticket context (generated)
│
├── business/                   → Business maps (command-generated)
│   ├── business-model.md          → Problem/solution, value prop      (/project-discovery Phase 1)
│   ├── domain-glossary.md         → Ubiquitous language, state machines (/project-discovery Phase 1)
...
│
├── infrastructure/             → Backend/frontend/infra maps (generated by /project-discovery Phase 3)
...
├── bug-screenshot-annotation/ → Turns a raw bug screenshot into QA-style annotated evidence (local HTML+CSS overlays). Auto-loaded inline by `/sprint-testing` Stage 2 for visual/positional bugs.
...
├── pr-review-lead/          → QA Lead / QA Architect review of a PR's test-automation work against KATA doctrine, citing a concrete doctrine reference or code location per finding. Strictness preflight first; never posts without explicit OK.
